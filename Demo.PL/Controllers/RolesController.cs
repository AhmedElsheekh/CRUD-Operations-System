    ...
    public class RolesController : Controller
    {
        ...
        public async Task<IActionResult> AddOrRemoveUser(string roleId)
        {
            ...
            foreach(var user in users)
            {
                ...
                userInRoleViewModels.Add(userInRole);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> AddOrRemoveUser(string roleId, List<UserInRoleViewModel> userInRoleViews)
        {
            var role = await _roleManager.FindByIdAsync(roleId);
            ...
            if(ModelState.IsValid)
            {
                foreach(var userView in userInRoleViews)
                {
                    var user = await _userManager.FindByIdAsync(userView.UserId);

                    if (userView.IsSelected && !await _userManager.IsInRoleAsync(user, role.Name))
                        await _userManager.AddToRoleAsync(user, role.Name);
                    else if (!userView.IsSelected && await _userManager.IsInRoleAsync(user, role.Name))
